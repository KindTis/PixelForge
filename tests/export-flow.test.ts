...
test("내보내기는 전부 성공할 때만 기존 대상 폴더를 교체한다", async () => {
  ...
  try {
    await assert.rejects(
      exportProject("godot", createDocument({ width: 1, height: 1 }), options, root, {
        build: async () => [
...
test("프로젝트 저장 실패는 새 대상을 제거하고 기존 대상 폴더를 복구한다", async () => {
  ...
  const project = createDocument({ width: 1, height: 1 });
...
test("대상 폴더 복구 실패는 원인과 복구 경로를 함께 보고한다", async () => {
  ...
  try {
    await assert.rejects(
      exportProject("common", createDocument({ width: 1, height: 1 }), options, root, {
        commit: async () => {
