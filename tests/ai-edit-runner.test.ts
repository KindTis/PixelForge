...
import { runAiEdit, selectionOverlay, selectionRuns, type AiEditExecutionState } from "../src/client/editor/ai-edit.ts";
import { ToolController } from "../src/client/editor/ToolController.ts";
...
test("AI 실행기는 13개 결정적 도구를 직접 포인터 제스처와 같게 실행한다", () => {
  for (const tool of Object.keys(validPoints) as EditorTool[]) {
    ...
    const application = runAiEdit(state, target, { summary: tool, actions: [{ tool, points }] });
...
test("AI 스프레이 결과는 대상 이미지 안의 픽셀을 만든다", () => {
  const document = createDocument({ width: 4, height: 4 });
  const target = targetOf(document);
  const originalRandom = Math.random;
  Math.random = () => 0.5;
  try {
    const application = runAiEdit(stateOf(document), target, { summary: "spray", actions: [{ tool: "spray", points: validPoints.spray }] });
    const changed = application.historySteps.at(-1)!;
    assert.ok(imageBytes(changed, target).some((channel) => channel !== 0));
    assert.equal(changed.images[changed.cels[celKey(target.frameId, target.layerId)].imageId].data.length, 4 * 4 * 4);
  } finally {
...
test("문서 좌표는 오프셋된 셀의 로컬 좌표로 변환된다", () => {
  ...
  const application = runAiEdit(stateOf(document), target, { summary: "offset", actions: [{ tool: "pencil", points: [{ x: 3, y: 2 }] }] });
...
test("연결 셀 편집은 대상 셀만 분리한다", () => {
  ...
  const application = runAiEdit(stateOf(document), target, { summary: "unlink", actions: [{ tool: "pencil", points: [{ x: 0, y: 0 }] }] });
...
test("잘못된 후속 동작은 입력 문서와 편집기 상태를 전혀 바꾸지 않는다", () => {
  ...
  assert.throws(() => runAiEdit(state, target, { summary: "invalid", actions }), /좌표 수/);
