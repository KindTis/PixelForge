...
export function ExportDialog({ settings: initial, onClose, onExport }: {
  ...
  onClose(): void;
  onExport(target: ExportTarget, settings: ExportSettings): Promise<ExportResult>;
}) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setBusy(true);
    setError("");
    try { setResult(await onExport(target, settings)); }
  ...
  return <dialog ref={dialog} className="export-dialog" aria-labelledby="export-title" onCancel={onClose}>
    <form onSubmit={submit}>
      <header><div><span>ENGINE DELIVERY</span><h2 id="export-title">게임 엔진으로 내보내기</h2></div><button type="button" onClick={onClose} aria-label="닫기">×</button></header>
      ...
      <p className="export-hint">모든 애니메이션 태그와 프레임 시간이 함께 기록됩니다.</p>
      ...
      <footer><button type="button" onClick={onClose}>닫기</button><button className="primary" type="submit" disabled={busy}>{busy ? "내보내는 중…" : `${target === "common" ? "범용" : target === "godot" ? "Godot" : "Unity"} 묶음 만들기`}</button></footer>
    </form>
  </dialog>;
}
